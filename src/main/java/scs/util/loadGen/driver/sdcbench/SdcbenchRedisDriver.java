	...
	private void startQuery(String scriptCommand,int serviceId){
		try {  
			...
			while((line = br.readLine()) != null ) {
				...
				data.setQueryTime95th((int)Float.parseFloat(split[5]));//取99分位数 命令行输出依次为 [QPS MEAN 0, 50, 80, 95, 99, 99.9, 100}]
				data.setQueryTime99th((int)Float.parseFloat(split[6]));//取99分位数 命令行输出依次为 [QPS MEAN 95th 99th 99.9th 100th]
				data.setQueryTime999th((int)Float.parseFloat(split[7]));//取99分位数 命令行输出依次为 [QPS MEAN 95th 99th 99.9th 100th]
